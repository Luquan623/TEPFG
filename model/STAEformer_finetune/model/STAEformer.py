...
class Model(nn.Module):
    def __init__(
        ...
        self.use_mixed_proj = opt.use_mixed_proj
    ...
    def forward(self, x):
        ...
        if self.dow_embedding_dim > 0:
            dow = x[..., 2]
        x = x[..., : self.input_dim]  # (8,12,307,3)

        x = self.input_proj(x)   # (batch_size, in_steps, num_nodes, input_embedding_dim),将3为特征映射成24维 (8,12,307,24)
        features = [x]
        if self.tod_embedding_dim > 0:
